...
function getSource() {
    ...
    try {
        ...
        if (dntShim.dntGlobalThis?.process?.version) {
            // deno-lint-ignore no-explicit-any
            const nodeVersion = dntShim.dntGlobalThis.process.version.replace("v", "");
            return `nodejs@${nodeVersion},${moduleSource}`;
        }
        // Assumes running in Deno instead
        return `deno@${dntShim.Deno.version.deno},${moduleSource}`;
    }
    catch {
